...
const addPost = async (req, res) => {
    try {
        ...
        const postDoc = await Post_1.default.create({
            title,
            slug: finalSlug,
            content,
        ...
        // Now populate safely
        const populatedPost = await Post_1.default.findById(postDoc._id)
            .populate("author", "name email")
            .populate("category", "name slug")
            .populate("tags", "name slug")
...
const updatePost = async (req, res) => {
    ...
    try {
        const existing = await Post_1.default.findById(id);
        if (!existing)
            return res.status(404).json({ error: "Post not found" });
        if (!req.user)
        ...
        if (req.file?.path)
            existing.featuredImage = req.file.path;
        await existing.save();
        // Populate using a new query
        const populated = await Post_1.default.findById(existing._id)
            .populate("author", "name email")
            .populate("category", "name slug")
            .populate("tags", "name slug")
            .exec();
